...
RUNNER_CONFIG=/data/apps/SnowplowRealeases/r75/snowplow-master/3-enrich/emr-etl-runner/config/combined_prod_restart.yml
RUNNER_ENRICHMENTS=/data/apps/SnowplowRealeases/r75/snowplow-master/3-enrich/config/enrichments
LOADER_CONFIG=/data/apps/SnowplowRealeases/r75/snowplow-master/3-enrich/emr-etl-runner/config/combined_prod_restart.yml
...
echo "Moving raw logs from failed folder to Global raw log folder " >>  $LOGFILE 2>&1
aws s3 cp s3://udmd-p-archive/failedRawLogs/FailedRawLog_02-14-17--02-30-01   s3://udmd-global-p-raw-logs --recursive >> $LOGFILE 2>&1
...
if [ $ret_val -ne 0 ]; then
    ...
    /usr/local/bin/aws s3 mv  s3://udmd-p-etl/processing/ s3://udmd-p-archive/failedRawLogs/$RAWLOGFOLDER/ --recursive  >> $LOGFILE 2>&1
         #check the damage
           ...
           cat  $LOGFILE | mailx -s " EmrEtl Job failure "  -r  noreply@unileversolutions.com  dbhatt@sapient.com
          exit $ret_val
fi
